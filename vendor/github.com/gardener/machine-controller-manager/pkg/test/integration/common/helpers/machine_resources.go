...
func (c *Cluster) CreateMachine(namespace string) error {
	_, err := c.McmClient.MachineV1alpha1().Machines(namespace).Create(&v1alpha1.Machine{
...
func (c *Cluster) CreateMachineDeployment(namespace string) error {
	...
	_, err := c.McmClient.MachineV1alpha1().MachineDeployments(namespace).Create(&v1alpha1.MachineDeployment{
		ObjectMeta: metav1.ObjectMeta{
